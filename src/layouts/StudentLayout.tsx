        ...
        renderItem={(item: any) => (
          <List.Item>
            <List.Item.Meta
              ...
              title={
                <a onClick={() => history.push("/student/notice")}>
                  {item.title}
                </a>
              }
              description={
...
const PasswordModal = React.memo(
  ({ visible, onCancel, onOk, loading }: any) => {
...
const StudentLayout: React.FC = () => {
  ...
  // 修改密码
  const { loading: changingPassword, run: runChangePassword } = useRequest(
    changePassword,
    {
      manual: true,
      onSuccess: (res: any) => {
  ...
  const refreshFunctions = useMemo(
    () => ({
      ...
      "/student/survey": () =>
        window.dispatchEvent(new CustomEvent("refreshSurvey")),
    }),
    []
  )
  ...
  const handleRefresh = useCallback(() => {
    const refreshFunction =
      refreshFunctions[location.pathname as keyof typeof refreshFunctions]
    if (refreshFunction) {
      refreshFunction()
    }
    ...
    <div>
      <ProLayout
        ...
        menuItemRender={(item, dom) => (
          <div onClick={() => item.path && history.push(item.path)}>{dom}</div>
        )}
        /* 面包屑渲染 */
