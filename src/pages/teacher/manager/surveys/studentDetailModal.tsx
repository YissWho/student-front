    ...
    >
      {loading ? (
        ...
        <Descriptions column={1} bordered>
          ...
          <Descriptions.Item label="就业意向">
            {response.future_plan_display}
          </Descriptions.Item>
          {response.future_plan === 0 && (
            <>
              <Descriptions.Item label="就业类型">
                {response.employment_type_display}
              </Descriptions.Item>
              <Descriptions.Item label="城市偏好">
                {response.city_preference_display}
              </Descriptions.Item>
              <Descriptions.Item label="期望薪资">
                {response.expected_salary_display}
              </Descriptions.Item>
              <Descriptions.Item label="就业前景看法">
                {response.job_market_view_display}
              </Descriptions.Item>
            </>
          )}
          {response.future_plan === 1 && (
            <>
              <Descriptions.Item label="考研类型">
                {response.study_type_display}
              </Descriptions.Item>
              <Descriptions.Item label="目标院校">
                {response.target_school}
              </Descriptions.Item>
              <Descriptions.Item label="备考情况">
                {response.study_plan_status_display}
              </Descriptions.Item>
            </>
          )}
