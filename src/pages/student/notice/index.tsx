...
const Notice: React.FC = () => {
  ...
  // 批量标记已读
  const { run: markMultipleAsRead, loading: markingMultiple } = useRequest(
    markAsRead,
    {
      manual: true,
      debounceWait: 1000,
  ...
  const handleMarkAllAsRead = () => {
    const unreadIds = notices
      .filter((notice: Notice) => !notice.is_read)
      .map((notice: Notice) => notice.id)
                    ...
                    children: (
                      <motion.div
                        ...
                        className={`${styles.noticeItem} ${
                          notice.is_read ? styles.read : styles.unread
                      ...
                      >
                        ...
                        >
                          <div className={styles.noticeHeader}>
                            <Text strong={!notice.is_read}>{notice.title}</Text>
                            {!notice.is_read && (
                              <Button
                                type="link"
                                size="small"
